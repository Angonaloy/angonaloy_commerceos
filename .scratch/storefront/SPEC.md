...
The storefront is a **separate React app** (`github.com/Angonaloy/angonaloy_storefront`, originally forked from `github.com/noorkarimmehedi/e-commerce`, built for one merchant) deployed on Vercel. It fetches all data from the **public API** on the existing Merchant-Suite Express backend — no direct database access.
...
The storefront is a **separate React + Vite + TypeScript app** (`github.com/Angonaloy/angonaloy_storefront`, originally forked from `github.com/noorkarimmehedi/e-commerce` / Stepprs Bangladesh single-tenant storefront). It is not part of the Merchant-Suite monorepo — it lives in its own repo and deploys independently on Vercel.
...
- **The dedicated storefront repo** (`github.com/Angonaloy/angonaloy_storefront`, originally forked from `github.com/noorkarimmehedi/e-commerce` built for Stepprs Bangladesh) is the single-tenant Angonaloy storefront. It fetches all data via runtime API calls using the handle. The cart, checkout, and Meta Pixel code already work.
